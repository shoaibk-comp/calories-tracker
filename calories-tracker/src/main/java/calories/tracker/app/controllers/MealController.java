...
public class MealController {
     ...
     * @return MealsDTO with the current page, total pages and the list of meals
     */
    @GetMapping
    ...
    public MealsDTO searchMealsByDate(
        ...
        long totalPages = resultsCount / 10 + (resultsCount % 10 > 0 ? 1 : 0);

        return new MealsDTO(pageNumber, totalPages, MealDTO.fromMeals(result.getResult()));
    }

    /**
     * Saves a list of meals - they can be either new or existing.
     *
     * @param principal the current logged in user
